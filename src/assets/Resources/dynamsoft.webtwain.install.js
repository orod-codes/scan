...
(function () {
  ...
  ERR_LNADenied = 'Access to the local scanning service is blocked.',
  ERR_LNANotAllowed = 'Please grant access to the local scanning service.';
  ...
  function isChrome145plus() {
    if (Dynamsoft.navInfoSync.bChrome || Dynamsoft.navInfoSync.bEdge) {
      let ver = parseInt(Dynamsoft.navInfoSync.strBrowserVersion);
    ...
    if (_this.DWT) {
      ...
      if (bHTML5) {
        if (bIE) {
          ...
          ObjString.push('If you\'re still seeing this dialog after installing the service, refer to the <a target="_blank" href="' + getLNA_FAQ_URL(true) + '">guide</a> for details.');
          ObjString.push('</div>');
  ...
  Dynamsoft._dcp_dwt_onclickDownloadButton = function() {
    ...
    if (el) {
      if (el.getAttribute("html5") == "1") {
        ...
        newDiv.innerHTML = 'After installation, click below to verify the service is running.';
  ...
  Dynamsoft.OnBrowserLNADenied = function(msg, bReturnToInstallDialog, bPromptPermisstion)  {
    ...
    if(bPromptPermisstion) {
      ...
      if(Dynamsoft.navInfoSync.bFirefox) {
        strCaption = 'Action Required';
      } else {
        strCaption = 'Permission Required';
      }

      ObjString = [
        '<div style="color:#323234;font:17px/34px">', ERR_LNANotAllowed, '</div>',
        Dynamsoft.navInfoSync.bChrome || Dynamsoft.navInfoSync.bEdge ? '<div style="color:#323234;font:17px/34px">Click "Allow" to grant access.</div>' : '',
        '<div style="color:#323234;font-size:17px;padding:12px 0;">For more detailed information, please refer to <a target="_blank" href="',
        strLNA_FAQ_URL,
        ,'">this article</a>.</div>',
      ].join('');
    } else {
      // Error
      strCaption = 'Access Denied';
      ObjString = [
        '<div style="color:#323234;font:17px/34px">', ERR_LNADenied, '</div>',
        Dynamsoft.navInfoSync.bChrome || Dynamsoft.navInfoSync.bEdge  ? '<div style="color:#323234;font:17px/34px">Please update your site settings to grant access.</div>' : '',
        '<div style="color:#323234;font-size:17px;padding:12px 0;">For more detailed information, please refer to <a target="_blank" href="',
        strLNA_FAQ_URL,
        ,'">this article</a>.</div>',
      ].join('');
